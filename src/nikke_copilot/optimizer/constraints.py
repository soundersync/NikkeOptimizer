...
# via NIKKE_COPILOT_MIN_SKILL_SUM env var for power users.
...
def effective_min_skill_sum() -> int:
    ...
    Reads ``NIKKE_COPILOT_MIN_SKILL_SUM`` at call time so power users can
    ...
    """
    raw = os.environ.get("NIKKE_COPILOT_MIN_SKILL_SUM")
...
def is_valid_team(
    ...
    *,
    ...
    which honors the ``NIKKE_COPILOT_MIN_SKILL_SUM`` env override.
    """
    members = list(team)
    base = (
