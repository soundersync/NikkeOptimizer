"""CLI entry point — `nikkecopilot <command>`.
...
app = typer.Typer(
    ...
    help="NikkeCopilot — PvP team optimization for Goddess of Victory: NIKKE.",
)
console = Console()
...
def set_username_cmd(
    ...
    Equivalent to setting ``NIKKE_COPILOT_USERNAME`` permanently — env
...
def show_config_cmd() -> None:
    ...
    if name:
        ...
        console.print(
            ...
            "Run [bold]nikkecopilot set-username <name>[/] to set one.[/]"
        )
...
def doctor(
    ...
    if not db_path.exists():
        ...
        if n_chars >= 200:
            ...
            warn(f"only {n_chars} characters — run `nikkecopilot refresh`")
        else:
            err("no characters — run `nikkecopilot refresh`")
        if n_owned > 0:
            ok(f"{n_owned} owned characters in roster")
        else:
            warn("no owned characters — run `nikkecopilot import-csv <path>`")
        if n_cubes > 0:
            ok(f"{n_cubes} cubes")
        else:
            warn("no cubes — run `nikkecopilot import-cubes <dir>`")
    ...
    if pl is None:
        warn(
            ...
            "Place .webp files at ~/Library/Application Support/NikkeCopilot/portraits/"
        )
    else:
    ...
    if name:
        ...
        warn(
            ...
            "Run `nikkecopilot detect-username --save` or `set-username <name>`"
        )
...
def web(
    ...
    library_dir: Optional[Path] = typer.Option(
        ...
        help=(
            ...
            "If omitted, auto-discovers from $NIKKE_COPILOT_PORTRAITS or "
    ...
    if library_dir is None:
        ...
        if library_dir is not None:
            ...
            console.print(
                ...
                "disabled. Pass --library or set NIKKE_COPILOT_PORTRAITS to enable.[/]"
            )

    fastapi_app = create_app(db_path=db, portrait_library=library_dir)
    url = f"http://{host}:{port}/"
    console.print(f"[bold green]NikkeCopilot UI[/] {url}")
...
def counter_cmd(
    ...
    Examples:
        nikkecopilot counter "Snow White" "Crown" "Modernia" "Red Hood" "Liter"
        nikkecopilot counter --capture 1 _   # underscore is a placeholder
    """
    from ..data.models import ArenaMatch
...
def resolve_cmd(
    ...
    if result is None:
        console.print(
            ...
            "in the encoded skill registry. Use `nikkecopilot skill-coverage` "
...
def counter_sp_cmd(
    ...
    Example:
        nikkecopilot counter-sp -c 1 -c 2 -c 3
    """
    ...
    if mode == "rookie":
        ...
        if not rec.attack and not rec.defense:
            console.print(
                ...
                "Run: [cyan]nikkecopilot import-csv <path/to/roster.csv>[/]"
            )
            raise typer.Exit(1)
